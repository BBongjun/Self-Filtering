...
def noisify(dataset='mnist', train_images = None, nb_classes=10, train_labels=None, noise_type=None, noise_rate=0, random_state=0, random_noise=None):
    ...
    if noise_type == 'instance':
        ...
        dataset = zip(data, y_train)
        train_noisy_labels = get_instance_noisy_label(noise_rate, dataset, y_train, nb_classes, feature_size, norm_std, random_noise)
        actual_noise_rate = np.array([True if train_noisy_labels[i]!=y_train[i] else False for i in range(len(y_train))]).mean()
